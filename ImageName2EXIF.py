...
def rename(filepath, filetype, current_folder):
    ...
    while not rename_success:
        try:
            print("renaming: " + filepath + " ---> " + current_folder + dir_seperator + str(
                dateTime_renamed) + filetype)
            ...
            print("renaming within exception: " + filepath + " ---> " + current_folder + dir_seperator + str(
                dateTime_renamed) + str(double_names_nr) + filetype)
            try:
                os.rename(filepath,
                          current_folder + dir_seperator + str(dateTime_renamed) + str(double_names_nr) + filetype)
                double_names_nr += 1
                rename_success = True
            except:
                double_names_nr += 1
...
def show_files_missing_EXIF():
    ...
    print("there were " + str(
        len(list_of_files_with_no_exif)) + "Pictures that i couldnt rename, because there was no EXIF-Information")
...
print("Your current directory is: " + current_working_directory)

list_of_all_dirs.append(current_working_directory)
check_for_subdirectories(current_working_directory)
